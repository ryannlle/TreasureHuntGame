 ...
 *  |   Monitors when the treasure  |     TreasureGame          |
...
{
    private TreasureButton button; //has a treasure button
    private TreasureGame game; //has a game

    //Constructor
    //Purpose: Create an instance of the current  EmptyButton and TreasureGame variable
    public TreasureButtonListener(TreasureButton button, TreasureGame game) 
    {
        this.button = button;
        this.game = game;
    }
    
    //Purpose: Unique action perfomed exlcusive to the treasure button method that will first reveal the card. If the game 
